...
"""
================================================================================
 📥 AEGISFETCH-CLI v3.7 (PRO EDITION - AUTO-ENCRYPTED SECURITY)
================================================================================
 Features:
 - 🎯 Saves directly to Internal Storage Root (/sdcard/AegisDownloads/)
 - 🔐 In-Code Auto-Updating SHA-256 Hash Password Security
...
# Save directly in Phone's Main Root Storage
...
def clean_url_query_params(url):
    """Strips tracking query parameters from social media links (TikTok, FB, Insta)."""
    if not url:
        return None
    clean = url.strip()
...
def get_target_directory():
    """Finds accessible Internal Storage root path."""
    possible_paths = [
...
def fetch_media(url, mode, save_dir, quality=None):
    clean_url = clean_url_query_params(url)
    if not clean_url:
        print(f"\n{Fore.RED}[✘] Invalid URL! Please enter a valid http:// or https:// link.{Style.RESET_ALL}")
        return
    ...
    try:
        ...
        print(f"\n{Fore.RED}[✘] Download Failed: {str(e)[:150]}{Style.RESET_ALL}")
...
def history_and_security_manager():
    ...
    print(f"  {Fore.YELLOW}[1]{Style.RESET_ALL} View Download History (List / Raw JSON)")
    print(f"  {Fore.RED}[2]{Style.RESET_ALL} Clear / Remove Files & History (Advanced)")
    ...
    if sub_choice == "1":
        print(f"\n{Fore.CYAN}{Style.BRIGHT}📂 VIEW HISTORY OPTIONS:{Style.RESET_ALL}")
        print(f"  {Fore.YELLOW}[1]{Style.RESET_ALL} View Formatted Log List")
        print(f"  {Fore.YELLOW}[2]{Style.RESET_ALL} View Raw JSON Data File ({HISTORY_FILE})")
        print(f"  {Fore.WHITE}[0]{Style.RESET_ALL} Back")
        
        view_choice = input(f"\n{Fore.CYAN}Choose Option (1-2): {Style.RESET_ALL}").strip()
        
        if view_choice == "1":
            print(f"\n{Fore.YELLOW}{Style.BRIGHT}📜 DOWNLOAD HISTORY LOG (FORMATTED):{Style.RESET_ALL}\n")
            if os.path.exists(HISTORY_FILE):
                with open(HISTORY_FILE, 'r', encoding='utf-8') as f:
                    history = json.load(f)
                    if not history:
            ...
            print(f"\n{Fore.YELLOW}{Style.BRIGHT}📄 RAW JSON FILE CONTENT ({HISTORY_FILE}):{Style.RESET_ALL}\n")
            if os.path.exists(HISTORY_FILE):
                with open(HISTORY_FILE, 'r', encoding='utf-8') as f:
                    try:
                        raw_data = json.load(f)
                        print(Fore.GREEN + json.dumps(raw_data, indent=4, ensure_ascii=False))
                    except Exception as e:
                        print(Fore.RED + f"Error reading JSON: {e}")
            else:
                print(Fore.RED + "No history file found.")

    elif sub_choice == "2":
        print(f"\n{Fore.RED}{Style.BRIGHT}🗑️ REMOVE / CLEAR OPTIONS:{Style.RESET_ALL}")
        print(f"  {Fore.YELLOW}[1]{Style.RESET_ALL} Clear JSON History Log Only")
        print(f"  {Fore.YELLOW}[2]{Style.RESET_ALL} Delete Downloaded Media Files Only (From {target_dir})")
        print(f"  {Fore.YELLOW}[3]{Style.RESET_ALL} Both (Clear JSON History + Delete Downloaded Media Files)")
        print(f"  {Fore.WHITE}[0]{Style.RESET_ALL} Cancel")
        ...
        if rem_choice == "1":
            if os.path.exists(HISTORY_FILE):
                ...
                print(f"\n{Fore.GREEN}✅ JSON History file cleared successfully!{Style.RESET_ALL}")
            else:
                print(f"\n{Fore.YELLOW}No JSON history file found.{Style.RESET_ALL}")
            ...
            if os.path.exists(target_dir):
                ...
                for filename in os.listdir(target_dir):
                    ...
                    try:
                        ...
                        pass
                print(f"\n{Fore.GREEN}✅ Successfully deleted {count} downloaded media file(s) from storage!{Style.RESET_ALL}")
            else:
                print(f"\n{Fore.YELLOW}Download directory not found.{Style.RESET_ALL}")
            ...
            if os.path.exists(target_dir):
                for filename in os.listdir(target_dir):
                    ...
                    try:
                        ...
                        pass
            print(f"\n{Fore.GREEN}✅ Both Modes Cleared! JSON History removed & {count} media file(s) deleted.{Style.RESET_ALL}")
        else:
            print(f"\n{Fore.GREEN}Action cancelled.{Style.RESET_ALL}")
